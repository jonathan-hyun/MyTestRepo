...
IOS_SDK_VERSION="16.1" # xcodebuild -showsdks
SWIFT_PROJECT_NAME="ForstaFrameworkProxy"
SWIFT_PROJECT_PATH="$SWIFT_PROJECT_NAME/$SWIFT_PROJECT_NAME.xcodeproj"
SWIFT_BUILD_PATH="$SWIFT_PROJECT_NAME/build"
SWIFT_OUTPUT_PATH="VendorFrameworks/forsta-framework-proxy"
XAMARIN_BINDING_PATH="ForstaFrameworkProxy.Binding"
NAMESPACE_NAME="ForstaBinding"

echo "Build iOS framework for simulator and device"
rm -Rf "$SWIFT_BUILD_PATH"
xcodebuild -sdk iphonesimulator$IOS_SDK_VERSION -project "$SWIFT_PROJECT_PATH" -configuration Release EXCLUDED_ARCHS="arm64"
...
echo "Copy Confirmit framework to the output folder"
cp -Rf "$FORSTA_PATH" "$SWIFT_OUTPUT_PATH"

echo "Generating binding api definition and structs"
sharpie bind --sdk=iphoneos$IOS_SDK_VERSION --output="$SWIFT_OUTPUT_PATH/XamarinApiDef" --namespace="$NAMESPACE_NAME" --scope="$SWIFT_OUTPUT_PATH/$SWIFT_PROJECT_NAME.framework/Headers/" "$SWIFT_OUTPUT_PATH/$SWIFT_PROJECT_NAME.framework/Headers/$SWIFT_PROJECT_NAME-Swift.h"

echo "Replace existing metadata with the udpated"
cp -Rf "$SWIFT_OUTPUT_PATH/XamarinApiDef/." "$XAMARIN_BINDING_PATH/$XAMARIN_BINDING_PATH/"
